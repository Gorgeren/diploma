...
if __name__ == "__main__":
    ...
    # pg.execute("""UPDATE users SET first_name = 'Жабин' WHERE first_name = 'Абдул'""")
    # pg.execute("""SELECT * FROM users WHERE first_name = 'Борис' AND middle_name = 'Борисович'""")
    # pg.execute("""INSERT INTO transactions (id, transaction_date, transaction_amount, person_id) VALUES (1, '2010-01-23 19:20:05', 3343.75, 1), (2, '1998-01-09 12:28:58', 9474.95, 2), (3, '1999-12-09 14:56:34', 9386.51, 3), (4, '2006-04-21 05:36:36', 4158.07, 4), (5, '2021-08-01 00:51:56', 9591.02, 5), (6, '2002-03-14 22:23:19', 205.34, 6), (7, '2018-11-26 17:00:59', 2864.81, 7), (8, '2022-05-23 07:50:28', 1941.77, 8), (9, '1992-12-26 01:11:58', 8835.73, 9), (10, '1995-05-29 12:31:13', 8960.98, 10);""")
    # pg.execute("""INSERT INTO users (id, first_name, middle_name, last_name, sex, balance) VALUES (1, 'Велвел', 'Андрианович', 'Воробейчиков', 1, 2778714.0), (2, 'Гликерия', 'Радиевна', 'Экономова', 0, 8418880.46), (3, 'Мария', 'Пафнутиевна', 'Чернятинская', 0, 4746012.98), (4, 'Илзе', 'Амвросиевна', 'Полувалова', 0, 5679217.07), (5, 'Алексина', 'Егориевна', 'Никитенко', 0, 7443199.42), (6, 'Оле', 'Ардальонович', 'Босяк', 1, 8412805.73), (7, 'Урсула', 'Аполлинариевна', 'Ларюхина', 0, 9192003.96), (8, 'Трифон', 'Исаевич', 'Марунин', 1, 846645.32), (9, 'Полактия', 'Созонтовна', 'Владимирская', 0, 1244156.08), (10, 'Рашад', 'Антропиевич', 'Русинович', 1, 7262073.2);""")
    # pg.execute("""SELECT * FROM users WHERE id BETWEEN 777 and 100000""")
    
    st = time.time()
    pg.del_table("users")
    pg.del_table("transactions")
    pg.execute("""SELECT COUNT(*) FROM users;""")
    end = time.time()
    print(f"time: {end - st}")
